...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if(message.mentions.users.first()) {
      ...
      let offline = client.emotes.offline;
      let online = client.emotes.online;
      let Idle = client.emotes.Idle;
      let dnd = client.emotes.dnd;
    
    if (target.presence.status === "dnd") target.presence.status = `Do Not Disturb ${dnd}`;
    if (target.presence.status === "idle") target.presence.status = `Idle ${Idle}`;
    if (target.presence.status === "online") target.presence.status = `Online ${online}`;
    if (target.presence.status === "offline") target.presence.status = `Offline ${offline}`;
